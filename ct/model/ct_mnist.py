...
class CT_MNIST(pl.LightningModule):
  def __init__(self, expl_coeff=2.0):
    super().__init__()
    
    self.expl_coeff = expl_coeff
  ...
  def test_step(self, batch, batch_idx):
    ...
    # Accuracy
    self.test_accuracy(pred_class.squeeze(), target_class.int())
  ...
  def loss_fn(self, target_class, target_concept, pred_class, attn):
    ...
    loss = cls_loss + self.expl_coeff*expl_loss
